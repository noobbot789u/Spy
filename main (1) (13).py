    ...
    if killed_players:
        ...
        if len(killed_players) == 1:
            ...
            if detective_also_killed:
                ...
                kill_announcement = f"{killed_role} {killed_role_emoji} - [{killed_name}](tg://user?id={killed_id}) was killed by 🤵🏻 The Spy"
        else:
...
async def handle_spy_kill_phase(context: ContextTypes.DEFAULT_TYPE, chat_id: int):
    ...
    if killed_players:
        ...
        if len(killed_players) == 1:
            ...
            kill_announcement = f"{killed_role} {killed_role_emoji} - {killed_name} was killed by 🤵🏻 The Spy"
        else:
            killed_info = []
            for kid in killed_players:
                kname = next(name for user_id, name in game.players if user_id == kid)
                krole = game.player_roles.get(kid, 'Citizen')
                krole_emoji = get_role_emoji(krole)
                killed_info.append(f"{krole} {krole_emoji} - {kname}")
