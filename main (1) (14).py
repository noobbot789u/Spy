...
async def auto_start_game(context: ContextTypes.DEFAULT_TYPE, chat_id: int):
    ...
    game_started_text = (
        "Be careful… not everyone here is as innocent as they seem.\n"
        "👀 The enemy might be sitting right next to you.\n"
        "💭 Truth is hidden… but lies are everywhere.\n\n"
        "💬 Start giving your hints in the chat.\n"
        "🕵️ Find the spy… and vote them out. 🗳️"
    )
    ...
    try:
        with open('start.png', 'rb') as photo:
            pinned_msg = await context.bot.send_photo(
                chat_id=chat_id,
                photo=photo,
...
async def start_round(context: ContextTypes.DEFAULT_TYPE, chat_id: int):
    ...
    try:
        ...
        alive_names = ", ".join(name for uid, name in alive_players)
        round_info_text = f"Round - {game.current_round}/5\nAlive - {alive_names}"
...
async def start_voting(context: ContextTypes.DEFAULT_TYPE, chat_id: int):
    ...
    chat_voting_message = f"Round {round_num} Voting 🗳️\n\nVoting has started! Click the Vote button to vote in private."
...
async def end_voting(context: ContextTypes.DEFAULT_TYPE, chat_id: int):
    ...
    if not game.votes or game.current_round >= 5:
        if game.current_round >= 5:
            ...
            await context.bot.send_message(
                chat_id=chat_id,
                text=f"🏡 Be careful, the night will be more darker now \n🏙️ The Spy is looking for next victim all over the city\n\nRound {next_round} will be very starting in few seconds"
            )
