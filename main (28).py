...
# Data file for scores (kept for reference, now using SQLite)
...
# Single global connection with lock for simplicity (bot is mostly single-threaded async)
_db_conn = None
    ...
    with _db_lock:
        conn.executescript('''
            CREATE TABLE IF NOT EXISTS player_scores (
                ...
                spy_wins INTEGER NOT NULL DEFAULT 0
            );
            ...
            CREATE TABLE IF NOT EXISTS group_settings (
                chat_id INTEGER PRIMARY KEY,
                turn_time INTEGER,
                discussion_time INTEGER,
                voting_time INTEGER,
                dead_people_can_write INTEGER,
                delete_word_messages INTEGER,
                round_time INTEGER
            );
        ''')
        conn.commit()
...
def db_load_all():
    ...
    global player_scores, banned_users, all_users, private_users, all_group_chats, group_settings, group_round_times
    conn = get_db_conn()
    with _db_lock:
        # Load player scores
        cursor = conn.execute('SELECT user_id, name, cash, games_played, games_won, spy_wins FROM player_scores')
        player_scores = {}
        for row in cursor:
            player_scores[row['user_id']] = {
                'name': row['name'],
                'cash': row['cash'],
        ...
        # Load banned users
        cursor = conn.execute('SELECT user_id FROM banned_users')
        banned_users = set(row['user_id'] for row in cursor)
        # Load user data
        cursor = conn.execute('SELECT user_id, is_private FROM user_data')
        for row in cursor:
            all_users.add(row['user_id'])
            if row['is_private']:
                private_users.add(row['user_id'])
        # Load group chats
        cursor = conn.execute('SELECT chat_id FROM group_chats')
        all_group_chats = set(row['chat_id'] for row in cursor)
        # Load group settings
        cursor = conn.execute('SELECT * FROM group_settings')
        for row in cursor:
            cid = row['chat_id']
            s = {}
            if row['turn_time'] is not None:
                s['turn_time'] = row['turn_time']
            if row['discussion_time'] is not None:
                s['discussion_time'] = row['discussion_time']
            if row['voting_time'] is not None:
                s['voting_time'] = row['voting_time']
            if row['dead_people_can_write'] is not None:
                s['dead_people_can_write'] = bool(row['dead_people_can_write'])
            if row['delete_word_messages'] is not None:
                s['delete_word_messages'] = bool(row['delete_word_messages'])
            if s:
                group_settings[cid] = s
            if row['round_time'] is not None:
                group_round_times[cid] = row['round_time']
    logger.info("Loaded all data from SQLite database")
...
def save_scores():
    conn = get_db_conn()
    with _db_lock:
        for key, player in player_scores.items():
            conn.execute('''
                INSERT INTO player_scores (user_id, name, cash, games_played, games_won, spy_wins)
                VALUES (?, ?, ?, ?, ?, ?)
                ON CONFLICT(user_id) DO UPDATE SET
                    ...
                    spy_wins=excluded.spy_wins
            ''', (key, player['name'], player['cash'], player['games_played'], player['games_won'], player['spy_wins']))
        conn.commit()
...
def save_user_data():
    ...
    with _db_lock:
        for user_id in all_users:
            is_private = 1 if user_id in private_users else 0
            conn.execute('INSERT OR IGNORE INTO user_data (user_id, is_private) VALUES (?, ?)', (user_id, is_private))
            conn.execute('UPDATE user_data SET is_private=? WHERE user_id=?', (is_private, user_id))
        for chat_id in all_group_chats:
            conn.execute('INSERT OR IGNORE INTO group_chats (chat_id) VALUES (?)', (chat_id,))
        conn.commit()
...
class GameState:
    def __init__(self):
        ...
        # New Timer variables
        self.lobby_end_time: Optional[float] = None
        self.timer_disabled: bool = False
...
# Per-group round time setting (persists across games, in seconds)
group_round_times = {}  # chat_id -> round_time_seconds
# Per-group settings file
GROUP_SETTINGS_FILE = os.getenv('GROUP_SETTINGS_FILE', 'group_settings.json')

# Per-group persistent settings storage
group_settings = {}  # chat_id -> {turn_time, discussion_time, voting_time, dead_people_can_write, delete_word_messages}

def load_group_settings():
    # Group settings are now loaded via db_load_all() - this function is kept for compatibility
    pass
...
def save_group_settings():
    try:
        conn = get_db_conn()
        with _db_lock:
            all_chat_ids = set(group_settings.keys()) | set(group_round_times.keys())
            for cid in all_chat_ids:
                s = group_settings.get(cid, {})
                conn.execute('''
                    INSERT INTO group_settings (chat_id, turn_time, discussion_time, voting_time, dead_people_can_write, delete_word_messages, round_time)
                    VALUES (?, ?, ?, ?, ?, ?, ?)
                    ON CONFLICT(chat_id) DO UPDATE SET
                        turn_time=excluded.turn_time,
                        discussion_time=excluded.discussion_time,
                        voting_time=excluded.voting_time,
                        dead_people_can_write=excluded.dead_people_can_write,
                        delete_word_messages=excluded.delete_word_messages,
                        round_time=excluded.round_time
                ''', (
                    cid,
                    s.get('turn_time'),
                    s.get('discussion_time'),
                    s.get('voting_time'),
                    int(s['dead_people_can_write']) if 'dead_people_can_write' in s else None,
                    int(s['delete_word_messages']) if 'delete_word_messages' in s else None,
                    group_round_times.get(cid)
                ))
            conn.commit()
    except Exception as e:
        logger.error(f"Failed to save group settings: {e}")

def apply_group_settings_to_game(chat_id: int, game: GameState):
    settings = group_settings.get(chat_id, {})
    if 'turn_time' in settings:
        game.turn_time = settings['turn_time']
    if 'discussion_time' in settings:
        game.discussion_time = settings['discussion_time']
    if 'voting_time' in settings:
        game.voting_time = settings['voting_time']
    if 'dead_people_can_write' in settings:
        game.dead_people_can_write = settings['dead_people_can_write']
    if 'delete_word_messages' in settings:
        game.delete_word_messages = settings['delete_word_messages']
    if chat_id in group_round_times:
        game.turn_time = group_round_times[chat_id]
...
def assign_words_for_round(game):
    ...
    for user_id, _ in game.players:
        ...
        if original_role == 'Spy':
            game.word_messages[user_id] = f"""You're {role_emoji}
Your word: {spy_word}
...
async def send_game_words_summary(context: ContextTypes.DEFAULT_TYPE, chat_id: int, game: GameState, winning_message: str = None, winning_parse_mode: str = None):
    """Send summary of all game words, then after 3 seconds send the winning message"""
    # Build the message
    ...
    # Wait 3 seconds then send winning message
    if winning_message:
        await asyncio.sleep(3)
        try:
            if winning_parse_mode:
                await context.bot.send_message(chat_id=chat_id, text=winning_message, parse_mode=winning_parse_mode)
            else:
                await context.bot.send_message(chat_id=chat_id, text=winning_message)
        except Exception as e:
            logger.error(f"Failed to send winning message after words summary: {e}")

async def auto_start_game(context: ContextTypes.DEFAULT_TYPE, chat_id: int):
    """Automatically starts the game similar to /begin command"""
    game = games.get(chat_id)
    if not game or not game.lobby_active or game.game_active or len(game.players) < 4:
        return
    ...
    apply_group_settings_to_game(chat_id, game)
    ...
    try:
        if game.host_message_id:
            await context.bot.delete_message(chat_id=chat_id, message_id=game.host_message_id)
    except:
        pass
    ...
    game_started_text = (
        ...
        f"Give clues about your word, but don't let spy know the real word!"
    )
    ...
    try:
        with open('start.gif', 'rb') as animation:
            pinned_msg = await context.bot.send_animation(
                chat_id=chat_id,
                animation=animation,
                caption=game_started_text,
...
async def lobby_timer_task(context: ContextTypes.DEFAULT_TYPE, chat_id: int):
    """Background task to handle lobby timeout"""
    import time
    while True:
        await asyncio.sleep(1)
        game = games.get(chat_id)
        
        # Exit if game was deleted, started, or no longer in lobby
        if not game or not game.lobby_active or game.game_active:
            return
            
        if game.timer_disabled:
            return
            
        if game.lobby_end_time and time.time() >= game.lobby_end_time:
            if len(game.players) < 4:
                try:
                    await context.bot.send_message(chat_id=chat_id, text="⏱️ Time over\n🎮 Not enough players to start the game")
                except:
                    pass
                remove_game(chat_id)
            else:
                await auto_start_game(context, chat_id)
            return
...
async def extend_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not game or not game.lobby_active:
        return

    # Check if user is host or admin
    is_admin = (user_id == game.admin_user_id) or (user_id in creators)
    if not is_admin:
        try:
    ...
    if not is_admin:
        await update.message.reply_text("Only group admins or the lobby host can extend the timer.")
        return

    if game.timer_disabled:
        await update.message.reply_text("Timer is currently disabled. The game will wait for /begin.")
        return

    if game.lobby_end_time:
        game.lobby_end_time += 30
        await update.message.reply_text("⏳ Join timer extended by 30 seconds!")

async def disabletimer_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    update_user_stats(update)
    if not update.effective_chat or not update.effective_user:
        return
    ...
    if not is_admin:
        await update.message.reply_text("Only group admins or the lobby host can disable the timer.")
        return

    game.timer_disabled = True
    await update.message.reply_text("🛑 Join timer disabled. The game will wait for /begin to start.")
...
async def leave_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Ignore if no lobby is active or if game is already running
    if not game or not game.lobby_active or game.game_active:
        return

    # Find if the user is in the players list
    player_index = next((i for i, p in enumerate(game.players) if p[0] == user_id), None)
    
    if player_index is None:
        # User isn't in the lobby, ignore silently
        return
        
    # Remove the player from the game
    game.players.pop(player_index)
    await update.message.reply_text("You have successfully left the game lobby.")
    
    # Update the lobby message
    if not game.players:
        players_list = "Nobody has joined yet."
    else:
        players_list = "\n".join([f"{i+1}) [{name}](tg://user?id={uid})" for i, (uid, name) in enumerate(game.players)])
    ...
    if game.host_message_id:
        try:
            ...
            logger.error(f"Failed to update lobby message on leave: {e}")
...
Use /begin to start the game"""
    ...
    for chat_id, game in games.items():
        if game.game_active and user_id in [p[0] for p in game.players]:
            # Command ignores players who are in an active game
            return
...
async def host_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Clean up any ghost game state left by background tasks after a game ends
    if not game.lobby_active and not game.game_active:
        if chat_id in games:
            del games[chat_id]
        game = get_or_create_game(chat_id)
    ...
    # Start the 2-minute timer for the lobby
    import time
    game.lobby_end_time = time.time() + 120
    game.timer_disabled = False
    asyncio.create_task(lobby_timer_task(context, chat_id))
...
def build_game_status_message(game: GameState) -> str:
    """Build the hints status message shown when a player gives a hint."""
    hints_lines = ""
    any_hints = False
    for player_id, player_name in game.players:
        ...
        if player_hints_list:
            ...
            hints_lines += f"➔ {player_name} - {last_hint}\n"
...
async def promote_new_spy(context: ContextTypes.DEFAULT_TYPE, game: GameState, chat_id: int):
    ...
    try:
        ...
        # Send public announcement to group chat
        try:
            await context.bot.send_message(
                chat_id=chat_id,
                text="Gangsters have appointed a new 🤵🏻 spy to the team"
            )
        except Exception as e:
            logger.error(f"Failed to send new spy announcement: {e}")
        ...
        for chat_id, game in games.items():
            if game.game_active and (user_id in game.spy_user_ids or user_id in game.gangster_ids) and user_id not in game.eliminated_players:
                ...
                if game.spy_kill_available:
                    ...
                    team_message = f"{sender_emoji} - {sender_name}: {message_text}"
...
async def begin_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Only host or group admins can use /begin — silently ignore everyone else
    caller_id = update.effective_user.id if update.effective_user else None
    if caller_id:
        is_host = (caller_id == game.admin_user_id)
        if not is_host:
            try:
                chat_member = await context.bot.get_chat_member(chat_id, caller_id)
    ...
    # Apply per-group persistent settings
    apply_group_settings_to_game(chat_id, game)
...
async def execute_detective_kill(context: ContextTypes.DEFAULT_TYPE, chat_id: int):
    ...
    if game.doctor_save_target == target_id:
        ...
        # Doctor saved message will be sent after spy kill notification in the correct order
        game.detective_kill_target = None
    ...
    kill_announcement = f"👨\u200d💼 {target_role} - {target_name} was killed during the night... 🕵️\u200d♂️ Detective was spotted at his house just before the death occurred"
    ...
    if target_id == game.kamikaze_id:
        ...
        kamikaze_revenge = f"{detective_emoji} - [{detective_name}](tg://user?id={game.detective_id}) was killed by Kamikaze 💣"
...
async def start_round(context: ContextTypes.DEFAULT_TYPE, chat_id: int):
    ...
    # Send round info message after 3 seconds (for all rounds)
    await asyncio.sleep(3)
    try:
        alive_players = [(uid, name) for uid, name in game.players if uid not in game.eliminated_players]
        total_players = len(game.players)
        alive_count = len(alive_players)

        # Build roles alive section (same as /alive command)
        alive_roles = set()
        for player_id, player_name in alive_players:
            role = game.player_roles.get(player_id, "Citizen")
            alive_roles.add(role)
        ...
        round_info_text = (
            ...
            f"Roles alive: {roles_alive_text}"
        )
        await context.bot.send_message(chat_id=chat_id, text=round_info_text)
    except Exception as e:
        logger.error(f"Failed to send round info message: {e}")
...
async def execute_spy_kill(context: ContextTypes.DEFAULT_TYPE, chat_id: int, detective_target_id=None, detective_killed_by_spy=False, detective_doctor_saved=False):
    ...
    # Get spy team kill target using majority vote when >1 spy team members voted
    spy_target = None
    
    # Collect votes from alive spy team members
    alive_spy_team = [sid for sid in game.spy_user_ids if sid not in game.eliminated_players]
    
    if len(alive_spy_team) > 1:
        # Multiple alive spy team members: use majority vote
        kill_vote_counts = {}
        for member_id in alive_spy_team:
            target = game.spy_kill_targets.get(member_id)
            if target:
                kill_vote_counts[target] = kill_vote_counts.get(target, 0) + 1
        
        if kill_vote_counts:
            max_votes = max(kill_vote_counts.values())
            top_targets = [t for t, v in kill_vote_counts.items() if v == max_votes]
            # If tied, pick the main spy's choice if available, else first in list
            spy_id = game.spy_user_ids[0] if game.spy_user_ids else None
            if spy_id and game.spy_kill_targets.get(spy_id) in top_targets:
                spy_target = game.spy_kill_targets[spy_id]
            else:
                spy_target = top_targets[0]
    else:
        # Single spy team member: use their choice directly
        spy_id = game.spy_user_ids[0] if game.spy_user_ids else None
        if spy_id:
            spy_target = game.spy_kill_targets.get(spy_id)
        # If main spy didn't choose, check gangsters
        if not spy_target:
            for gangster_id in game.gangster_ids:
    ...
    # --- Send kill notifications in order with 2s delays ---
    # Order: detective kill (already sent), 2s, spy kill, 2s, kamikaze kill, doctor saved last

    if not killed_players and not doctor_saved_someone:
        # No one was killed and doctor didn't save anyone
        await context.bot.send_message(
            chat_id=chat_id,
            text="No one was killed in the deadly night\n\n🌆 The City is safe"
        )
    
    if killed_players:
        # Check for mutual kill (spy & detective both killed each other)
        if detective_also_killed:
            mutual_msg = "Both came to their door 🔪💥"
            try:
                await context.bot.send_message(chat_id=chat_id, text=mutual_msg)
            except Exception as e:
                logger.error(f"Failed to send mutual kill message: {e}")
        
        # Send spy kill notification for each spy-killed player
        spy_killed = [kid for kid in killed_players if not (detective_also_killed and kid == game.detective_id)]
        for kid in spy_killed:
            kname = next((name for uid, name in game.players if uid == kid), "Unknown")
            krole = game.player_roles.get(kid, 'Citizen')
            spy_kill_msg = f"👨\u200d💼 {krole} - {kname} was killed during the night... 🤵🏻 Spy was spotted at his house just before the death occurred"
            try:
                with open('102.jpg', 'rb') as photo:
                    await context.bot.send_photo(chat_id=chat_id, photo=photo, caption=spy_kill_msg)
            except Exception as e:
                try:
                    await context.bot.send_message(chat_id=chat_id, text=spy_kill_msg)
                except Exception as e2:
                    logger.error(f"Failed to send spy kill announcement: {e2}")
        
        # Wait 2 seconds then handle Kamikaze
        ...
        if kamikaze_killed_by_spy_team:
            ...
            # Kamikaze kill notification
            kamikaze_kill_msg = f"👨\u200d💼 {killer_role} - {killer_name} was killed during the night... 💣 Kamikaze was spotted at his house just before the death occurred"
            try:
                await context.bot.send_message(chat_id=chat_id, text=kamikaze_kill_msg)
            except Exception as e:
                logger.error(f"Failed to send kamikaze kill announcement: {e}")
            
            # Kamikaze wins when killed by spy team (not voted out)
            kamikaze_name = next((name for uid, name in game.players if uid == game.kamikaze_id), "Kamikaze")
            update_player_score(game.kamikaze_id, kamikaze_name, won=True, won_as_spy=False, coins=coins_reward)
            
            try:
                await context.bot.send_message(
                    chat_id=chat_id,
                    text=f"🥷 Kamikaze [{kamikaze_name}](tg://user?id={game.kamikaze_id}) wins! They were killed but took a spy with them! 💣",
                    parse_mode='Markdown'
                )
            except Exception as e:
                logger.error(f"Failed to send kamikaze win announcement: {e}")

    # Wait 2 seconds then send doctor saved message if applicable (last in order)
    if doctor_saved_someone or detective_doctor_saved:
        await asyncio.sleep(2)
        try:
            await context.bot.send_message(
                chat_id=chat_id,
                text="🧑‍⚕️ Doctor saved someone from death 💉"
            )
        except Exception as e:
            logger.error(f"Failed to send doctor saved message: {e}")
...
async def execute_kamikaze_revenge(context: ContextTypes.DEFAULT_TYPE, chat_id: int, game: GameState):
    ...
    kamikaze_kill_announcement = f"{target_emoji} - [{target_name}](tg://user?id={target_id}) was killed by Kamikaze 💣"
...
async def start_voting(context: ContextTypes.DEFAULT_TYPE, chat_id: int):
    ...
    detective_target_id, detective_killed_by_spy, detective_doctor_saved = await execute_detective_kill(context, chat_id)
    
    # Wait 2 seconds after detective kill notification
    await asyncio.sleep(2)
    
    # Execute Spy kill SECOND - pass detective kill info
    await execute_spy_kill(context, chat_id, detective_target_id, detective_killed_by_spy, detective_doctor_saved)
    
    # Wait 2 seconds after all kill notifications
    await asyncio.sleep(2)
    ...
    if len(alive_spies) == 0:
        ...
        for uid, name in all_citizens:
            if uid not in game.eliminated_players:
                ...
                role_emoji = get_role_emoji(role)
                winners_with_roles.append(f"[{name}](tg://user?id={uid}) - {role_emoji}")
        ...
        for spy_id in game.spy_user_ids:
            ...
            losers_with_roles.append(f"[{spy_name}](tg://user?id={spy_id}) - {spy_emoji}")
        # Add dead citizens
        for uid, name in all_citizens:
            if uid in game.eliminated_players:
                role = game.original_roles.get(uid, 'Citizen')
                role_emoji = get_role_emoji(role)
                losers_with_roles.append(f"[{name}](tg://user?id={uid}) - {role_emoji}")
        ...
        # Send words summary first, then winning message after 3 seconds
        asyncio.create_task(send_game_words_summary(context, chat_id, game, game_end_message, 'Markdown'))
    ...
    if len(alive_citizens) <= len(alive_spies):
        ...
        for spy_id in game.spy_user_ids:
            if spy_id not in game.eliminated_players:
                ...
                spy_role = game.original_roles.get(spy_id, 'Spy')
                spy_emoji = get_role_emoji(spy_role)
                spy_winners_with_roles.append(f"[{spy_name}](tg://user?id={spy_id}) - {spy_emoji}")
    ...
    discussion_message = (
        ...
        f"Discuss with others and vote for the Spy\n\n"
        f"Voting will start in {game.discussion_time} seconds"
    )
    # Send discussion message with animation
    try:
        with open('dis.gif', 'rb') as animation:
            await context.bot.send_animation(chat_id=chat_id, animation=animation, caption=discussion_message)
    except Exception as e:
        logger.error(f"Failed to send animation dis.gif: {e}")
    ...
    # Send voting buttons to each player via DM
    bot_info = await context.bot.get_me()
    bot_username = bot_info.username
    bot_dm_link = f"https://t.me/{bot_username}"

    voting_keyboard_dm = []
    for v_user_id, v_name in alive_players_after_kill:
        voting_keyboard_dm.append([InlineKeyboardButton(v_name, callback_data=f"vote_{v_user_id}", api_kwargs={"style": "primary"})])

    dm_reply_markup = InlineKeyboardMarkup(voting_keyboard_dm)
    dm_voting_message = f"Round {round_num} Voting 🗳️\n\nSelect the person you want to vote for:"

    for v_user_id, v_name in alive_players_after_kill:
        try:
            await context.bot.send_message(
                chat_id=v_user_id,
                text=dm_voting_message,
                reply_markup=dm_reply_markup
            )
        except Exception as e:
            logger.error(f"Failed to send voting DM to {v_user_id}: {e}")

    # Send "Vote" button in chat (green style) that opens bot DM
    chat_vote_keyboard = [
        [InlineKeyboardButton("Vote 🗳️", url=bot_dm_link, api_kwargs={"style": "success"})]
    ]
    chat_vote_markup = InlineKeyboardMarkup(chat_vote_keyboard)
    chat_voting_message = f"🗳️ Round {round_num} Voting\n\nVote for the spy, click the button below to vote"

    await context.bot.send_message(
        chat_id=chat_id,
        text=chat_voting_message,
        reply_markup=chat_vote_markup
    )
...
async def handle_spy_kill_phase(context: ContextTypes.DEFAULT_TYPE, chat_id: int):
    ...
    if killed_players:
        ...
        if len(killed_players) == 1:
            ...
            kill_announcement = f"{killed_role_emoji} - {killed_name} was killed by The Spy"
        else:
            killed_info = []
            for kid in killed_players:
                ...
                killed_info.append(f"{krole_emoji} - {kname}")
                
            kill_announcement = f"{', '.join(killed_info)} were killed by 🤵🏻 The Spy"
        ...
        # No one was killed
        kill_announcement = "No one was killed in the deadly night\n\n🌆 The City is safe"
    ...
    if len(alive_spies) >= len(alive_citizens):
        # Spies win - only reward ALIVE spies
        coins_reward = 50
        for spy_id in game.spy_user_ids:
            spy_name = next(name for user_id, name in game.players if user_id == spy_id)
            if spy_id not in game.eliminated_players:
    ...
    # CHANGE 6: Send discussion message with animation
    try:
        with open('dis.gif', 'rb') as animation:
            await context.bot.send_animation(chat_id=chat_id, animation=animation, caption=discussion_message)
    except Exception as e:
...
async def send_voting_options_after_delay(context: ContextTypes.DEFAULT_TYPE, chat_id: int):
    try:
        ...
        # Build voting keyboard for DM
        dm_keyboard = []
        for uid, player_name in alive_players:
            dm_keyboard.append([InlineKeyboardButton(player_name, callback_data=f"vote_{uid}", api_kwargs={"style": "primary"})])

        dm_reply_markup = InlineKeyboardMarkup(dm_keyboard)
        dm_text = "🗳️ Voting time! Please vote who you think is the Spy:"

        # Send voting buttons to each player via DM
        for uid, player_name in alive_players:
            try:
                await context.bot.send_message(chat_id=uid, text=dm_text, reply_markup=dm_reply_markup)
            except Exception as e:
                logger.error(f"Failed to send voting DM to {uid}: {e}")

        # Send green "Vote" button in chat
        bot_info = await context.bot.get_me()
        bot_dm_link = f"https://t.me/{bot_info.username}"
        chat_keyboard = [[InlineKeyboardButton("Vote 🗳️", url=bot_dm_link, api_kwargs={"style": "success"})]]
        chat_markup = InlineKeyboardMarkup(chat_keyboard)

        await context.bot.send_message(
            chat_id=chat_id,
            text="🗳️ Voting time! Click the Vote button to vote in private.",
...
async def send_detective_inspect_result(context: ContextTypes.DEFAULT_TYPE, game: GameState, target_id: int):
    ...
    try:
        ...
        if target_id in game.eliminated_players:
            ...
            role_emoji = get_role_emoji(role)
            result_message = f"{target_name} is a {role_emoji}"
...
async def end_voting(context: ContextTypes.DEFAULT_TYPE, chat_id: int):
    ...
    if not game.votes or game.current_round >= 5:
        if game.current_round >= 5:
            ...
            # Spies won at round 5 - only reward ALIVE spies
            for spy_id in game.spy_user_ids:
                spy_name = next(name for user_id, name in game.players if user_id == spy_id)
                if spy_id not in game.eliminated_players:
                    update_player_score(spy_id, spy_name, won=True, won_as_spy=True, coins=coins_reward)
            ...
            await context.bot.send_message(
                chat_id=chat_id,
                text="Next round starts in few seconds ⏳"
            )
    ...
    if not vote_counts:
        ...
        if game.current_round >= 5:
            # Spies win in round 5 - only reward ALIVE spies
            for spy_id in game.spy_user_ids:
                spy_name = next(name for user_id, name in game.players if user_id == spy_id)
                if spy_id not in game.eliminated_players:
                    update_player_score(spy_id, spy_name, won=True, won_as_spy=True, coins=coins_reward)
    ...
    if len(most_voted) > 1:
        if game.current_round >= 5:
            # Round 5 tie - spies win; only reward ALIVE spies
            for spy_id in game.spy_user_ids:
                spy_name = next(name for user_id, name in game.players if user_id == spy_id)
                if spy_id not in game.eliminated_players:
                    update_player_score(spy_id, spy_name, won=True, won_as_spy=True, coins=coins_reward)
    ...
    if eliminated_player_id in game.spy_user_ids:
        ...
        if not remaining_spies:
            ...
            losers_list = "\n".join([f"[{name}](tg://user?id={uid}) - {emoji}" for uid, name, role, emoji in losers_with_roles])

            # FIXED: Show role in bold HTML
            voting_result_msg = f"Voting is over ⏳\n\n\n{eliminated_player_name} was <b>{eliminated_role_emoji}</b>"
            ...
            try:
                ...
                await context.bot.send_message(
                    chat_id=chat_id,
                    text=f"Voting is over ⏳\n\n\n{eliminated_player_name} was {eliminated_role_emoji}"
                )
            ...
            # Some spies still remain after this vote
            # FIXED: Show role in bold HTML
            voting_result_msg = f"Voting is over ⏳\n\n\n{eliminated_player_name} was <b>{eliminated_role_emoji}</b>"
            ...
            # Round 5: if a spy/gangster was voted out and spies still remain → alive spies win immediately
            if game.current_round >= 5:
                remaining_spies_r5 = [spy_id for spy_id in game.spy_user_ids if spy_id not in game.eliminated_players]
                for spy_id in game.spy_user_ids:
                    spy_name = next(name for user_id, name in game.players if user_id == spy_id)
                    if spy_id not in game.eliminated_players:
                        update_player_score(spy_id, spy_name, won=True, won_as_spy=True, coins=coins_reward)
                ...
                all_citizens_r5 = [(uid, name) for uid, name in game.players if uid not in game.spy_user_ids]
                for uid, name in all_citizens_r5:
                    update_player_score(uid, name, won=False, won_as_spy=False, coins=0)
                
                spy_winners_r5 = []
                for spy_id in remaining_spies_r5:
                    spy_name = next(name for user_id, name in game.players if user_id == spy_id)
                    spy_role = game.original_roles.get(spy_id, 'Spy')
                    spy_emoji = get_role_emoji(spy_role)
                    spy_winners_r5.append(f"[{spy_name}](tg://user?id={spy_id}) - {spy_emoji}")
                
                losers_r5 = []
                for uid, name in all_citizens_r5:
                    role = game.original_roles.get(uid, 'Citizen')
                    role_emoji = get_role_emoji(role)
                    losers_r5.append(f"[{name}](tg://user?id={uid}) - {role_emoji}")
                for spy_id in game.spy_user_ids:
                    if spy_id in game.eliminated_players:
                        spy_name = next(name for user_id, name in game.players if user_id == spy_id)
                        spy_role = game.original_roles.get(spy_id, 'Spy')
                        spy_emoji = get_role_emoji(spy_role)
                        losers_r5.append(f"[{spy_name}](tg://user?id={spy_id}) - {spy_emoji}")
                
                game_end_message_r5 = (
                    f"🎉 Spy won! 🎉\n\n"
                    f"Winners :\n\n"
                    f"{chr(10).join(spy_winners_r5)}\n\n"
                    f"Losers :\n\n"
                    f"{chr(10).join(losers_r5)}"
                )
                
                await notify_game_end(context, chat_id, "Spies", game.current_round)
                asyncio.create_task(send_game_words_summary(context, chat_id, game, game_end_message_r5, 'Markdown'))
                remove_game(chat_id)
                return
    else:
...
async def kamikaze_revenge_timeout(context: ContextTypes.DEFAULT_TYPE, game: GameState, chat_id: int):
    """Handle Kamikaze revenge timeout after 30 seconds"""
    await asyncio.sleep(30)
...
async def continue_to_next_round(context: ContextTypes.DEFAULT_TYPE, chat_id: int):
    ...
    if game.current_round > 5:
        ...
        if alive_spies:
            # Spies survived 5 rounds - only ALIVE spies win
            for spy_id in game.spy_user_ids:
                spy_name = next(name for user_id, name in game.players if user_id == spy_id)
                if spy_id not in game.eliminated_players:
                    update_player_score(spy_id, spy_name, won=True, won_as_spy=True, coins=50)
...
async def button_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("detective_inspect_"):
        ...
        # Send public message to group chat about detective checking someone
        for gc_id, g in games.items():
            if g is user_game:
                try:
                    await context.bot.send_message(
                        chat_id=gc_id,
                        text="🕵️\u200d♂️ Detective went to check ID at someone's door, and found out something mysterious"
                    )
                except Exception as e:
                    logger.error(f"Failed to send detective inspect public message: {e}")
                break
    ...
    if data.startswith("kamikaze_revenge_"):
        ...
        import time
        if user_game.kamikaze_revenge_timestamp and (time.time() - user_game.kamikaze_revenge_timestamp) > 30:
        ...
        await query.edit_message_text(f"💣 You will take {target_name} with you to death in 5 seconds!")
        await query.answer()
        
        # Execute revenge after 5 seconds
    ...
    if data.startswith("kill_"):
        ...
        team_notification = f"{selector_role_emoji} - {selector_name} has selected {target_name} to kill"
    ...
    if data == "main_time_settings":
        ...
        group_settings[chat_id]['dead_people_can_write'] = True
        save_group_settings()
        ...
        if chat_id not in group_settings:
            group_settings[chat_id] = {}
        group_settings[chat_id]['dead_people_can_write'] = False
        save_group_settings()
        ...
        if chat_id not in group_settings:
            group_settings[chat_id] = {}
        group_settings[chat_id]['delete_word_messages'] = True
        save_group_settings()
        ...
        if chat_id not in group_settings:
            group_settings[chat_id] = {}
        group_settings[chat_id]['delete_word_messages'] = False
        save_group_settings()
        ...
        current = group_round_times.get(chat_id)
        current_label = {60: "1 minute", 90: "1:30 minutes", 120: "2 minutes", 180: "3 minutes"}.get(current, "Default (auto)")
        keyboard = [
            [InlineKeyboardButton("1 minute (60s)", callback_data="set_round_time_60")],
            [InlineKeyboardButton("1:30 minutes (90s)", callback_data="set_round_time_90")],
            [InlineKeyboardButton("2 minutes (120s)", callback_data="set_round_time_120")],
            [InlineKeyboardButton("3 minutes (180s)", callback_data="set_round_time_180")],
            [InlineKeyboardButton("Back to Settings", callback_data="back_to_main_settings")]
        ]
        reply_markup = InlineKeyboardMarkup(keyboard)
        await query.edit_message_text(
            f"⏳ Change Rounds time\n\nCurrent setting: {current_label}\n\nSelect a round duration (applies to the next game):",
            reply_markup=reply_markup
        )
        await query.answer()
        return

    elif data in ["set_round_time_60", "set_round_time_90", "set_round_time_120", "set_round_time_180"]:
        try:
            chat_member = await context.bot.get_chat_member(chat_id, user_id)
            if chat_member.status not in ['administrator', 'creator']:
                await query.answer("Only group admins can change settings!", show_alert=True)
        ...
        time_map = {
            "set_round_time_60": 60,
            "set_round_time_90": 90,
            "set_round_time_120": 120,
            "set_round_time_180": 180
        }
        label_map = {
            "set_round_time_60": "1 minute",
            "set_round_time_90": "1:30 minutes",
            "set_round_time_120": "2 minutes",
            "set_round_time_180": "3 minutes"
        }
        group_round_times[chat_id] = time_map[data]
        label = label_map[data]
        save_group_settings()
        keyboard = [[InlineKeyboardButton("Back to Settings", callback_data="back_to_main_settings")]]
        reply_markup = InlineKeyboardMarkup(keyboard)
        await query.edit_message_text(
            f"✅ Round timer set to {label}!\n\nThis will apply when the next game starts.",
            reply_markup=reply_markup
        )
        await query.answer("Round time updated!")
        return
        ...
        keyboard = [
            [InlineKeyboardButton("⏳ Change Rounds time", callback_data="change_round_time")],
        ...
        if chat_id not in group_settings:
            group_settings[chat_id] = {}
        group_settings[chat_id][setting_type] = time_value
        save_group_settings()
    ...
    if data == "check_word":
        ...
        if user_id in game.spy_user_ids or user_id in game.gangster_ids:
            word_message = f"Your role: {role_emoji}\nYour word: {game.spy_word}"
        else:
        ...
        # Find the game this user is participating in (voting now happens via DM)
        user_game = None
        user_game_chat_id = None
        for g_chat_id, g in games.items():
            if g.voting_active and user_id in [p[0] for p in g.players]:
                user_game = g
                user_game_chat_id = g_chat_id
                break

        if not user_game or not user_game.voting_active:
            await query.answer("Voting is not active right now!", show_alert=True)
            return

        voted_for_user_id = int(data.split("_")[1])

        if user_id in user_game.eliminated_players:
            await query.answer("You are eliminated and cannot vote!", show_alert=True)
            return

        if user_id in user_game.votes:
            await query.answer(text="You have already voted! You can only vote once.", show_alert=True)
            return

        user_game.votes[user_id] = voted_for_user_id

        voter_name = next(name for uid, name in user_game.players if uid == user_id)
        voted_for_name = next(name for uid, name in user_game.players if uid == voted_for_user_id)

        await context.bot.send_message(
            chat_id=user_game_chat_id,
...
async def oldscore_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Creator command: migrate all old JSON data to new SQLite DB without any data loss"""
    if not update.effective_user:
        return
    ...
    await update.message.reply_text("Starting migration of old JSON data to SQLite database...")

    migrated = []
    errors = []

    # 1. Migrate scores.json
    try:
        if os.path.exists(SCORES_FILE):
            ...
            if isinstance(old_scores, dict):
                conn = get_db_conn()
                with _db_lock:
                    for uid, data in old_scores.items():
                        if isinstance(data, dict):
                            conn.execute('''
                                ...
                                ON CONFLICT(user_id) DO UPDATE SET
                                    name=excluded.name,
                                    cash=MAX(player_scores.cash, excluded.cash),
                                    games_played=MAX(player_scores.games_played, excluded.games_played),
                                    games_won=MAX(player_scores.games_won, excluded.games_won),
                                    spy_wins=MAX(player_scores.spy_wins, excluded.spy_wins)
                            ''', (
                                str(uid),
                    ...
                    conn.commit()
                # Reload in-memory scores
                db_load_all()
                migrated.append(f"scores.json: {len(old_scores)} records")
    except Exception as e:
        errors.append(f"scores.json: {e}")

    # 2. Migrate banned_users.json
    try:
        if os.path.exists(BANNED_USERS_FILE):
            with open(BANNED_USERS_FILE, 'r') as f:
                old_banned = json.load(f)
            old_banned_list = old_banned.get('banned_users', [])
            conn = get_db_conn()
            with _db_lock:
                for uid in old_banned_list:
                    conn.execute('INSERT OR IGNORE INTO banned_users (user_id) VALUES (?)', (int(uid),))
                conn.commit()
            banned_users.update(int(uid) for uid in old_banned_list)
            migrated.append(f"banned_users.json: {len(old_banned_list)} records")
    except Exception as e:
        errors.append(f"banned_users.json: {e}")

    # 3. Migrate user_data.json
    try:
        if os.path.exists(USER_DATA_FILE):
            with open(USER_DATA_FILE, 'r') as f:
                old_user_data = json.load(f)
            old_all_users = old_user_data.get('all_users', [])
            old_private_users = set(old_user_data.get('private_users', []))
            old_group_chats = old_user_data.get('all_group_chats', [])
            conn = get_db_conn()
            with _db_lock:
                for uid in old_all_users:
                    is_private = 1 if int(uid) in old_private_users else 0
                    conn.execute('INSERT OR IGNORE INTO user_data (user_id, is_private) VALUES (?, ?)', (int(uid), is_private))
                for cid in old_group_chats:
                    conn.execute('INSERT OR IGNORE INTO group_chats (chat_id) VALUES (?)', (int(cid),))
                conn.commit()
            all_users.update(int(uid) for uid in old_all_users)
            private_users.update(int(uid) for uid in old_private_users)
            all_group_chats.update(int(cid) for cid in old_group_chats)
            migrated.append(f"user_data.json: {len(old_all_users)} users, {len(old_group_chats)} groups")
    except Exception as e:
        errors.append(f"user_data.json: {e}")

    # 4. Migrate group_settings.json if it exists
    try:
        GROUP_SETTINGS_FILE_OLD = os.getenv('GROUP_SETTINGS_FILE', 'group_settings.json')
        if os.path.exists(GROUP_SETTINGS_FILE_OLD):
            with open(GROUP_SETTINGS_FILE_OLD, 'r') as f:
                old_gs = json.load(f)
            old_group_settings = {int(k): v for k, v in old_gs.get('group_settings', {}).items()}
            old_round_times = {int(k): v for k, v in old_gs.get('group_round_times', {}).items()}
            group_settings.update(old_group_settings)
            group_round_times.update(old_round_times)
            save_group_settings()
            migrated.append(f"group_settings.json: {len(old_group_settings)} group settings")
    except Exception as e:
        errors.append(f"group_settings.json: {e}")
    ...
    if not migrated and not errors:
        result += "No old JSON files found to migrate."
...
async def checkgame_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Creator-only: show total number of active games across all groups."""
    if not update.effective_user or not update.message:
        return
    if update.effective_user.id not in creators:
        return
    total_active = sum(1 for g in games.values() if g.lobby_active or g.game_active)
    await update.message.reply_text(f"total active games: {total_active}")


async def endthis_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Creator-only: immediately end the game in the current group."""
    if not update.effective_user or not update.message:
        return
    if update.effective_user.id not in creators:
        return
    if not update.effective_chat or update.effective_chat.type == 'private':
        await update.message.reply_text("Use this command in a group to end the game there.")
        return
    chat_id = update.effective_chat.id
    game = games.get(chat_id)
    if not game or (not game.lobby_active and not game.game_active):
...
def main():
    ...
    application.add_handler(CommandHandler("disabletimer", disabletimer_command))
    ...
    application.add_handler(CommandHandler("oldscore", oldscore_command))
    ...
    application.add_handler(CommandHandler("Checkgame", checkgame_command))
    application.add_handler(CommandHandler("Endthis", endthis_command))
